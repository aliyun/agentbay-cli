...
2. **scope / 命令组加粗前缀**：`**apikey**: ...`（借鉴上游 SDK 风格，比按 scope 二级分组更紧凑）；中文段对 CLI 命令相关条目优先按命令组归类（如 `apikey`、`image`、`docker`、`skills`、`network`、`core/auth`），无法归入命令组或属于全局能力 / 基础设施 / 发版流程的改动，可归为“全局”“安全合规”“RAM 权限”等用户可理解的主题。
...
统一译法：
  ...
  context → 上下文
  flag → 参数 / 选项（不要用"标志"）
  feat / fix / docs / refactor → 中文段分类标题要翻译为“功能 / 缺陷修复 / 文档 / 重构”，不要保留英文类别名
```
...
> 注意：修改并提交 `CHANGELOG.md` 只会改变仓库中的事实源，**不会自动更新 GitHub 上已存在的 Release body**。已发布 Release 需要显式执行 backfill 才会同步。
...
# 3. 先预览，再同步刷新已发 release 的 body（从 CHANGELOG 抽取）
bash scripts/backfill-release-notes.sh --dry-run --tag v0.4.0
...
1. **CHANGELOG.md 历史段补齐**：用 `make release-prep` 思路对 v0.1.0 ~ v0.3.0 共约 15 个版本逐个生成双语段并 prepend（一次性手工 + AI 翻译），最终一次 commit 把整份补齐版本的 CHANGELOG.md 推上去。历史版本也必须保持 `### English` + `### 中文` 双语结构，不能为了回灌只保留中文。
2. **新版本发布优先**：如果同一批变更中既包含新版本（如 v0.4.0）又包含历史整理，先推送包含 CHANGELOG 的代码并完成新版本 Release / tag workflow，确认新 Release 正常后，再处理历史 release body 同步。
3. **release body 同步**：先执行 `bash scripts/backfill-release-notes.sh --dry-run` 预览全量回灌内容；确认无误后再执行 `bash scripts/backfill-release-notes.sh`（无参数，全量回灌）从 CHANGELOG.md extract 每个版本段，推到对应 release。
...
| ---------- | --------------------------- | -------------------------------------------- | ---------------- |
| `feat`     | 🚀 Features / 功能          | 用户可感知的新功能、新命令、新参数           | ✅               |
| `fix`      | 🐞 Bug Fixes / 缺陷修复     | 修 bug                                       | ✅               |
